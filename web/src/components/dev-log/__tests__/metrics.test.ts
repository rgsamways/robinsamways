...
function validSnapshotJson(overrides: Record<string, unknown> = {}) {
...
describe("parseMetricsSnapshots", () => {
  test("parses a well-formed snapshot", () => {
    const result = parseMetricsSnapshots(validSnapshotJson());
  ...
  test("falls back to the change name when label is missing", () => {
    const result = parseMetricsSnapshots(validSnapshotJson({ label: undefined }));
    expect(result[0].label).toBe("some-change");
  });

  test("defaults note to an empty string when missing", () => {
    const result = parseMetricsSnapshots(validSnapshotJson({ note: undefined }));
    expect(result[0].note).toBe("");
  });

  test("throws when the top-level value is not an array", () => {
    expect(() => parseMetricsSnapshots(JSON.stringify({ not: "an array" }))).toThrow(
      "metrics.json must contain an array of snapshots"
    );
  });

  test("throws when a required numeric field is missing or wrong type", () => {
    expect(() => parseMetricsSnapshots(validSnapshotJson({ code: "not a number" }))).toThrow(
      /non-numeric "code"/
    );
  });

  test("throws when date or change is missing", () => {
    expect(() => parseMetricsSnapshots(validSnapshotJson({ date: undefined }))).toThrow(
  ...
  test("parses multiple snapshots in order", () => {
    const json = JSON.stringify([
      JSON.parse(validSnapshotJson({ change: "first" }))[0],
      JSON.parse(validSnapshotJson({ change: "second" }))[0],
    ]);
    const result = parseMetricsSnapshots(json);
