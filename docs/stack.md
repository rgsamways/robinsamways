...
- `next/font/google` (Inter, Archivo Black, Poppins, Playfair Display, Work Sans) (2026-07-24) — loaded only in `web/src/components/prototype/fonts.ts` for the `/prototype/homepage-drawer` design-system-handoff mock's theme toggle: Inter for the "Handoff" theme, Archivo Black (headings) + Poppins (body) for an invented "Ad" theme (violet/red, poster-style), Playfair Display italic (headings) + Work Sans (body) for a "Vocare" theme modeled on vocare.ca's actual look. The real site stays JetBrains Mono-only. None of these are wired into `app/layout.tsx` or a production dependency unless/until the handoff scaffold is actually adopted.
